...
const AddPenguinForm = () => {
  ...
  const handleSubmit = (values: Penguin) => {
    ...
    if (values.secretKey !== import.meta.env.VITE_APP_SECRET_KEY) {
    ...
    .then(data => {
      console.log(data);
      message.success('Penguin data added successfully');
    })
    .catch(error => {
        ...
        <Form onFinish={values => handleSubmit(values)} layout="vertical">
            ...
            <Form.List name="lastPosition" initialValue={penguin.lastPosition}>
                ...
                <>
                    ...
                    <Form.Item
                        {...field}
                        label={`Last Position ${index + 1}`}
                        key={field.key}
                        rules={[{ required: true, message: 'Please input the last position!' }]}
                    >
                        <Input type="number" />
                    </Form.Item>
                    ))}
